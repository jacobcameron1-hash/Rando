 ...
 *
 * UpdateFeeShareConfigParams = { feeClaimers, payer, baseMint, additionalLookupTables? }
 */
export async function buildFeeShareSetupTransactions(
  ...
  const txResults = await sdk.feeShareAdmin.getUpdateConfigTransactions({
    baseMint: new PublicKey(baseMint),
    feeClaimers,
    payer: new PublicKey(launcherWallet),
  });

  return txResults.map(
...
export async function claimFeesForVault(
  ...
  for (const position of relevant) {
    try {
      const claimTxArray: Transaction[] = await sdk.fee.getClaimTransaction(
        vaultKeypair.publicKey,
        position,
      );
 ...
 */
export async function getVaultBalance(vaultPublicKey: string): Promise<number> {
...
/**
 * Build the admin lock transaction. Returns base64-serialized VersionedTransaction.
 * TransferFeeShareAdminParams = { baseMint, currentAdmin, newAdmin, payer }
 */
export async function buildAdminLockTransaction(
  ...
  const sdk = getSDK();
  const { transaction } = await sdk.feeShareAdmin.getTransferAdminTransaction({
    baseMint: new PublicKey(baseMint),
    currentAdmin: new PublicKey(currentAdmin),
    newAdmin: new PublicKey(SYSTEM_PROGRAM_ADDRESS),
    payer: new PublicKey(currentAdmin),
  });
  return Buffer.from((transaction as VersionedTransaction).serialize()).toString('base64');
}
...
export async function getTokenDecimals(mintAddress: string): Promise<number> {
  ...
  if (!info.value) throw new Error(`Mint not found: ${mintAddress}`);
  const parsed = (
