...
export async function POST(req: NextRequest) {
  try {
    ...
    const FIXED_PAYOUT_UI = Number(
      process.env.RANDO_PAYOUT_UI_AMOUNT || 1000000
    );
    ...
    const senderAta = await getAssociatedTokenAddress(mintPk, treasuryPk);
    const winnerAta = await getAssociatedTokenAddress(mintPk, winnerPk);
    ...
    const senderBalance = await connection
      .getTokenAccountBalance(senderAta)
      .catch(() => null);
